...
RegisterNUICallback('escape', function(data, cb)
	if hasIdentity then
		...
		TriggerEvent('chat:addMessage', { args = { '^1[IDENTITY]', '^1Musisz stworzyc postac aby grac' } })
	end
end)
...
function verifyName(name)
	...
	if nameLength > 25 or nameLength < 2 then
		return 'Twoje imie/nazwisko postaci jest za krotkie albo za dlugie.'
	end
	...
	if count ~= nameLength then
		return 'Twoja postac uzywa specjalnych znakow ktore sa zakazane na serwerze.'
	end
	...
	if spacesInName > 2 then
		return 'Twoje imie zawiera wiecej niz dwie spacje'
	end
	
	if spacesWithUpper ~= spacesInName then
		return 'Twoje imie musi sie zaczynac z duzej litery.'
	end
