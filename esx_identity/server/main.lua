...
AddEventHandler('esx_identity:setIdentity', function(data, myIdentifiers)
	setIdentity(myIdentifiers.steamid, data, function(callback)
		if callback then
			...
			TriggerClientEvent('chat:addMessage', source, { args = { '^[IDENTITY]', 'Nie udalo sie stworzyc postaci, sprobuj ponownie pozniej lub skontaktuj sie z adminsitratorem serwera!' } })
		end
	end)
end)
...
TriggerEvent('es:addCommand', 'register', function(source, args, user)
	getCharacters(source, function(data)
		if data.firstname3 ~= '' then
			TriggerClientEvent('chat:addMessage', source, { args = { '^[IDENTITY]', 'Mozesz posiadac tylko 3 postacie. Uzyj ^3/chardel^0  zeby usunac jakas postac.' } })
		else
			TriggerClientEvent('esx_identity:showRegisterIdentity', source, {})
		end
	end)
...
TriggerEvent('es:addGroupCommand', 'char', 'user', function(source, args, user)
	getIdentity(source, function(data)
		if data.firstname == '' then
			TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY]', 'Nie posiadasz aktywnej postaci!' } })
		else
			TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY]', 'Aktywna postac: ^2' .. data.firstname .. ' ' .. data.lastname } })
		end
	end)
...
TriggerEvent('es:addGroupCommand', 'charlist', 'user', function(source, args, user)
	getCharacters(source, function(data)
		if data.firstname1 ~= '' then
			TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY] Postac 1:', data.firstname1 .. ' ' .. data.lastname1 } })
			
			if data.firstname2 ~= '' then
				TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY] Postac 2:', data.firstname2 .. ' ' .. data.lastname2 } })
				
				if data.firstname3 ~= '' then
					TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY] Postac 3:', data.firstname3 .. ' ' .. data.lastname3 } })
				end
			end
		else
			TriggerClientEvent('chat:addMessage', source, { args = { '^[IDENTITY]', 'Musisz zarejestrowac postac. Uzyj komendy ^3/register^0 aby sie zarejestrowac.' } })
		end
	end)
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Brak permisji!' } })
...
TriggerEvent('es:addGroupCommand', 'charselect', 'user', function(source, args, user)
	...
	if charNumber == nil or charNumber > 3 or charNumber < 1 then
		TriggerClientEvent('chat:addMessage', source, { args = { '^[IDENTITY]', 'Niewlasciwa postac!' } })
		return
	end
	...
	getCharacters(source, function(data)
		if charNumber == 1 then
			...
			if data.firstname ~= '' then
				updateIdentity(GetPlayerIdentifiers(source)[1], data, function(callback)
					if callback then
						TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY]', 'Zmieniles postac na ^2' .. data.firstname .. ' ' .. data.lastname } })
					else
						TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY]', 'Nie udalo sie zaktualizowac postaci, sprobuj ponownie pozniej lub skontaktuj sie z adminsitratorem serwera!' } })
					end
				end)
			else
				TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY]', 'Nie posiadasz postaci w pierwszym slocie 1!' } })
			end
		elseif charNumber == 2 then
			...
			if data.firstname ~= '' then
				...
				TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY]', 'Nie posiadasz postaci w slocie numer 2!' } })
			end
		elseif charNumber == 3 then
			...
			if data.firstname ~= '' then
				...
				TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY]', 'Nie posiadasz postaci w slocie numer 3!' } })
			end
		else
			TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY]', 'Nie udalo sie zaktualizowac postaci, sprobuj ponownie pozniej lub skontaktuj sie z adminsitratorem serwera!' } })
		end
...
TriggerEvent('es:addGroupCommand', 'chardel', 'user', function(source, args, user)
	...
	getCharacters(source, function(data)
		...
		if charNumber == 1 then
			...
			if data.firstname ~= '' then
				deleteIdentity(GetPlayerIdentifiers(source)[1], data, function(callback)
					if callback then
						TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY]', 'Usunales ^1' .. data.firstname .. ' ' .. data.lastname } })
					else
						TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY]', 'Nie udalo sie usunac postaci, sprobuj ponownie pozniej lub skontaktuj sie z adminsitratorem serwera!' } })
					end
				end)
			else
				TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY]', 'Nie posiadasz postaci w slocie numer 1!' } })
			end
		...
		elseif charNumber == 3 then
			...
			TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY]', 'Nie udalo sie usunac postaci, sprobuj ponownie!' } })
		end
	end)
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Brak permisji!' } })
